...
var spr_jav_reg = preload("res://_asset/_texture/Items/Weapons/javelin_3_new.png")
var spr_jav_atk = preload("res://_asset/_texture/Items/Weapons/javelin_3_new_attack.png")
...
func _physics_process(delta):
	if target_array.size() > 0:
		...
		var distance_dif = global_position - player.global_position #玩家與標槍之間的距離
		var return_speed = 20
		if abs(distance_dif.x) > 500 or abs(distance_dif.y) > 500:
			#abs = 絕對值 / 如果有一個負值，就會把它變成正值
			return_speed = 100
...
func process_path():
	...
	var new_rotation_degress = angle.angle() + deg_to_rad(135) # 弧度為單位
	tween.tween_property(self,"rotation",new_rotation_degress, 0.25).set_trans(Tween.TRANS_QUINT).set_ease(Tween.EASE_OUT)
	tween.play()
...
func _on_reset_pos_timer_timeout(): #閒置狀態中的標籤會圍繞在玩家身邊
	var choose_direction =  randi() % 4
	#print("randi() % 4=", choose_direction)
